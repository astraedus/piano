...
function KeyDetailPanel({ keyId, depth }: { keyId: KeyId; depth: KeyDepth }) {
  ...
  return (
    <div className="space-y-5 text-sm">
      ...
      <div>
        <p className="text-xs uppercase tracking-[0.18em] text-[color:var(--ink-3)] mb-2">scale</p>
        <div className="flex items-baseline gap-2 flex-wrap">
          {scaleNotes.map((n, i) => (
            <span key={i} className="font-mono text-[color:var(--ink-2)]">{n.replace(/\d+$/, "")}</span>
          ))}
          <button
            type="button"
            onClick={async () => { await ensureAudio(); await playSequence(scaleNotes); }}
            className="ml-auto text-xs px-2 py-0.5 rounded-full border border-[color:var(--rule)] hover:border-[color:var(--accent-soft)] text-[color:var(--ink-3)] hover:text-[color:var(--accent)]"
          >
            hear it
          </button>
        </div>
      ...
      <div>
        <p className="text-xs uppercase tracking-[0.18em] text-[color:var(--ink-3)] mb-2">triad</p>
        <div className="flex items-baseline gap-2">
          <span className="font-mono text-[color:var(--ink-2)]">{triadNotes.map((n) => n.replace(/\d+$/, "")).join(" · ")}</span>
          <button
            type="button"
      ...
      <div>
        <p className="text-xs uppercase tracking-[0.18em] text-[color:var(--ink-3)] mb-2">{meta.mode === "major" ? "I – IV – V – I" : "i – iv – V – i"}</p>
        <button
          type="button"
          onClick={async () => { await ensureAudio(); await playProgression(prog); }}
          className="text-xs px-3 py-1 rounded-full border border-[color:var(--rule)] hover:border-[color:var(--accent-soft)] text-[color:var(--ink-3)] hover:text-[color:var(--accent)]"
        >
      ...
      <div>
        ...
        {piecesInKey.length === 0 ? (
          <p className="text-[color:var(--ink-3)] italic">none yet.</p>
        ) : (
